...
package happi.mv_dp_satisfaction_socprop_1_0_3;
...
/**
 * Job: MV_DP_SATISFACTION_SOCPROP_1 Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class MV_DP_SATISFACTION_SOCPROP_1 implements TalendJob {
	...
	private final String jobName = "MV_DP_SATISFACTION_SOCPROP_1";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					MV_DP_SATISFACTION_SOCPROP_1.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(MV_DP_SATISFACTION_SOCPROP_1.this,
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:9E0iwnKLhaqtAh9T8Kdqo+jtMKN/jBVPcAwAVw==");
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		final static byte[] commonByteArrayLock_HAPPI_MV_DP_SATISFACTION_SOCPROP_1 = new byte[0];
		static byte[] commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1 = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1.length) {
					if (length < 1024 && commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1.length == 0) {
						commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1 = new byte[1024];
					} else {
						commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1 = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1, 0, length);
				strReturn = new String(commonByteArray_HAPPI_MV_DP_SATISFACTION_SOCPROP_1, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_HAPPI_MV_DP_SATISFACTION_SOCPROP_1) {
...
// ###############################
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:Ilslth+ROUDF0bZT7fOOcejkQsPd/OTc8PGb7GYWzrF7YJu6IWKQRg==");
				...
				String dbquery_tDBInput_1 = "\nselect\ncount(*) as Repondu,\n(select    \ncount(*) \n	from \n	(\n	select  \n	case \n		when b.calltypename = 'Appel-B"
						...
						+ "'\n		when b.calltypename = 'Appel-Chatenay-Malabry' 	then 'CHATENAY-MALABRY'\n		when b.calltypename in  ('Appel-Seine Ou"
						+ "est','Appel-Chaville_Antenne') 	then 'BOULOGNE-BILLANCOURT'\n		when b.calltypename = 'Appel-Clichy' 			then 'CLICHY'\n		"
						+ "when b.calltypename = 'Appel-Nanterre'			then 'NANTERRE'\n		when b.calltypename = 'Appel-Plessis-Robinson' 	then 'LE PLE"
						+ "SSIS ROBINSON'\n		when b.calltypename = 'Appel-Rueil' 			then 'RUEIL-MALMAISON'\n		when b.calltypename = 'Appel-Suresnes"
						+ "_Carnot' 		then 'SURESNES (HORS ANTENNE)'	\n		when b.calltypename = 'Appel-Suresnes_Antenne' 		then 'ANTENNE SURESNES'	"
						+ "\n		else calltypename\n	end as calltypename,\n	case \n		when a.waitresolution = 1 then 'Répondu'\n		--when a.waitresoluti"
						+ "on = 2 then 'Abandonné par l appelant'\n		--when a.waitresolution = 3 then 'Manqué suite indisponibilité'\n		else 'Autre"
						+ "s'\n	end as type	\n	from informix.callrecord a\n	left join informix.calltypes b on a.calltypekey = b.calltypekey\n	where"
						+ " a.contacttype  in (1,6,8)\n	and a.calltypekey not in (0,6)\n	and MONTH(a.callstart) = month(TODAY) -1\n	) aut\nwhere au"
						+ "t.type = 'Autres'\nand rep.calltypename = aut.calltypename\n) as Autres\n	from \n	(\n	select  \n	--TO_CHAR(a.callstart, "
						+ "'%Y-%m-%d') as mydate,\n	case \n		when b.calltypename = 'Appel-Bagneux' 			then 'BAGNEUX'\n		when b.calltypename in ('Ap"
						+ "pel-Bois-Colombes',\n			'Appel-Villeneuve')		then 'BOIS-COLOMBES'\n		when b.calltypename = 'Appel-Chatenay-Malabry' 	the"
						+ "n 'CHATENAY-MALABRY'\n		when b.calltypename in  ('Appel-Seine Ouest', \n			'Appel-Chaville_Antenne') 		then 'BOULOGNE-BI"
						+ "LLANCOURT'\n		when b.calltypename = 'Appel-Clichy' 			then 'CLICHY'\n		when b.calltypename = 'Appel-Nanterre'			then 'NA"
						+ "NTERRE'\n		when b.calltypename = 'Appel-Plessis-Robinson' 	then 'LE PLESSIS ROBINSON'\n		when b.calltypename = 'Appel-Ru"
						+ "eil' 			then 'RUEIL-MALMAISON'\n		when b.calltypename = 'Appel-Suresnes_Carnot' 		then 'SURESNES (HORS ANTENNE)'	\n		whe"
						+ "n b.calltypename = 'Appel-Suresnes_Antenne' 		then 'ANTENNE SURESNES'	\n		else calltypename\n	end as calltypename,\n	cas"
						+ "e \n		when a.waitresolution = 1 then 'Répondu'\n		--when a.waitresolution = 2 then 'Abandonné par l appelant'\n		--when "
						+ "a.waitresolution = 3 then 'Manqué suite indisponibilité'\n		else 'Autres'\n	end as type	\n	from informix.callrecord a\n	"
						+ "left join informix.calltypes b on a.calltypekey = b.calltypekey\n	where a.contacttype  in (1,6,8)\n	and a.calltypekey no"
						+ "t in (0,6)\n	and MONTH(a.callstart) = month(TODAY) -1\n	) rep\nwhere rep.type = 'Répondu'\nand rep.calltypename  = '"
...
// ###############################
				...
				String dbquery_tDBInput_8 = "\nselect upper(LB_QUALIF1), \nCD_LOCALI1, \nCD_PATRIM1,\ncount(*) as NB from AFFAIRE a \nwhere year(DT_CREAAFF) = datep"
						...
						+ "OP = 1\ngroup by LB_QUALIF1, CD_LOCALI1, CD_PATRIM1\n";
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tDBInput_2 = "\nselect Origine,  sum(nbCourrierRep) as nbCourrierRep, sum(NbDelaiOK) as NbDelaiOK\nfrom\n(\nselect \nCASE v.[Libellé "
						...
						+ "hen 'BOULOGNE-BILLANCOURT'\n	when 'DPSO' 				then 'BOULOGNE-BILLANCOURT'\n	when 'DP Chatenay Malabry' 	then 'CHATENAY-MA"
						+ "LABRY'\n	when 'DP Clichy' 			then 'CLICHY'\n	when 'DP Nanterre' 			then 'NANTERRE'\n	when 'DP Plessis Robinsson' then 'L"
						+ "E PLESSIS ROBINSON'\n	when 'DP Rueil-Malmaison'	then 'RUEIL-MALMAISON'\n	when 'DP Suresnes'			then 'SURESNES (HORS ANTEN"
						+ "NE)'\n	else v.[Libellé Dossier Origine]\nend as Origine,\ncount(*) as nbCourrierRep,\n(select count(*)\nfrom V_TDB_COURR"
						+ "IERS_ENTRANTS v2\nwhere v2.[Année Numérisation] = datepart(year, dateadd(mm, -1, GETDATE()))\nand v2.[Mois Numérisation]"
						+ " = datepart(month, dateadd(mm, -1, GETDATE()))\nand v2.[Répondu (O/N)] ='O'\nand v2.[Délai Réponse Réalisé] <= 10\nand v"
						+ "2.[Libellé Dossier Origine]  = v.[Libellé Dossier Origine]) as NbDelaiOK\nfrom V_TDB_COURRIERS_ENTRANTS v\nwhere v.[Anné"
						+ "e Numérisation] = datepart(year, dateadd(mm, -1, GETDATE()))\nand v.[Mois Numérisation] =datepart(month, dateadd(mm, -1,"
						+ " GETDATE()))\nand v.[Répondu (O/N)] ='O'\ngroup by v.[Libellé Dossier Origine]\n) as base\nwhere base.Origine = '"
						+ context.Nom + "'\ngroup by Origine\n";
...
// ###############################      
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBInput_7 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:aY3OlgNGomz9y4gXJd8XeFEbuY8Vh1ZLV279aA==");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBInput_9 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:R+zcPxo+7BIbmhOWwMJNdaHMlWm+a1PS5EzP7Q==");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBInput_11 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:gF2jXwWQju6y6X+fYZQnDBhqy/EnIHz/oB8P9w==");
	...
	public static void main(String[] args) {
		final MV_DP_SATISFACTION_SOCPROP_1 MV_DP_SATISFACTION_SOCPROP_1Class = new MV_DP_SATISFACTION_SOCPROP_1();

		int exitCode = MV_DP_SATISFACTION_SOCPROP_1Class.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = MV_DP_SATISFACTION_SOCPROP_1.class.getClassLoader().getResourceAsStream(
					"happi/mv_dp_satisfaction_socprop_1_0_3/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = MV_DP_SATISFACTION_SOCPROP_1.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : MV_DP_SATISFACTION_SOCPROP_1");
		}
...
/************************************************************************************************
 * 450787 characters generated by Talend Open Studio for Data Integration on the
 * 9 février 2023 11:11:46 CET
 ************************************************************************************************/